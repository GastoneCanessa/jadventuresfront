...
export default function LoginFormParty() {
    const navigate = useNavigate();
    const [data, setData] = useAtom(party);
            ...
            .catch((error) => {
                if (error.response) {
                    if (error.response.status == 401) {
                        setErrorMessage("Invalid credentials. Please check your party name and authentication seal.");
                    } else if (error.response.status == 404) {
                        setErrorMessage("Guild not found. Please check the party name.");
                    } else {
                        setErrorMessage("Invalid credentials. Please check your party name and authentication seal.");
                    }
    ...
    return (
        <div className="container ">
            <form>
                <div className="mb-3">
                    <label className="form-label"><h4>Party Name</h4></label>
